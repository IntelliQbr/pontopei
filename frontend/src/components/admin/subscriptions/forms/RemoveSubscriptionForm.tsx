...
export function RemoveSubscriptionForm({
	subscription,
	onSuccess,
	onCancel,
}: RemoveSubscriptionFormProps) {
	...
	async function onSubmit() {
		...
		try {
			await AdminSubscriptionsApi.remove(subscription.id);
		...
		<div className="space-y-4">
			<div className="bg-destructive/10 p-1 rounded flex items-center gap-2">
				...
				<p className="text-destructive text-sm">
					Ao confirmar a remoção, todos os dados da assinatura serão
