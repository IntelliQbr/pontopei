...
import { School } from "@/models/interfaces/school/school.interface"; // Assume this interface exists or create it
...
export type AdminEditSchoolFormData = z.infer<typeof editSchoolFormSchema>;

interface AdminEditSchoolFormProps {
	school: School;
	onSuccess: () => void;
}

export function AdminEditSchoolForm({
	school,
	onSuccess,
}: AdminEditSchoolFormProps) {
	const form = useForm<AdminEditSchoolFormData>({
		resolver: zodResolver(editSchoolFormSchema),
		defaultValues: {
			name: school.name || "",
			address: school.address || "",
			directorId: school.createdById || "",
		},
	});

	const router = useRouter();

	async function onSubmit(data: AdminEditSchoolFormData) {
		try {
			await AdminSchoolsApi.update(school.id, data);
			onSuccess();
			router.refresh();
	...
	const { data: directors, isPending: isLoadingDirectors } = useQuery({
		queryKey: ["directors-edit-school"],
		queryFn: async () => {
