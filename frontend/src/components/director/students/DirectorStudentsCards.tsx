...
interface DirectorStudentsCardsProps {
	searchParams: DirectorStudentsSearchParams;
}

export async function DirectorStudentsCards({
	searchParams,
}: DirectorStudentsCardsProps) {
	const page = searchParams.page_students ?? 1;

	const take = 12;
	const skip = page ? (page - 1) * take : 0;
	const { students, total } = await StudentsApi.findAllDirectorStudents({
		skip,
		take,
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-4 mt-4 md:overflow-y-auto md:flex-1 items-start md:pr-2">
				{students.map((student) => (
					<StudentCard
						key={student.id}
						student={student}
						actionPath={`/dashboard/director/students/${student.id}`}
						isDirector={true}
					/>
				))}
			</div>
