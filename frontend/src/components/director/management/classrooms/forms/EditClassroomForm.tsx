...
const editClassroomFormFormSchema = z.object({
...
export type EditClassroomFormFormData = z.infer<
	typeof editClassroomFormFormSchema
>;
...
export function EditClassroomForm({
	...
	const form = useForm<EditClassroomFormFormData>({
		resolver: zodResolver(editClassroomFormFormSchema),
		defaultValues: {
			name: classroom.name,
	...
	async function onSubmit(data: EditClassroomFormFormData) {
		try {
			await api.put(`/classrooms/${classroom.id}`, data);
			onSuccess();
			router.refresh();
			toast.success("Sala de aula editada com sucesso");
		} catch (error) {
			toast.error("Erro ao editada sala de aula", {
	...
	const { data: schools, isPending } = useQuery({
		queryKey: ["schools-edit-classroom"],
		queryFn: () =>
